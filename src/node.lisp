...
(defmethod transform-items ((node node) pulled)
  (handler-case
      (iter:iterate
        ...
        (iter:collect (funcall (trans-fn node) item)))
    (error (c)
...
(defmethod run-iteration ((node node))
  (handler-case
    ...
    (error (c)
      (vom:error "node ~a had an unexpected error ~a" (node-name node) c)
      `(:error ,c))
    (:no-error (res) res)))
